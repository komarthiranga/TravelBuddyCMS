...
import { updateAttraction } from '@/master/attraction/api/updateAttraction'
...
export async function updateAttractionAction(
    ...
    const id = parseAttractionId(readString(formData, 'id'))

    if (!id) {
        return { error: 'This attraction could not be updated.' }
    }
    ...
    try {
        await updateAttraction(id, result.values)
    } catch (error) {
    ...
    revalidatePath(`/attraction/${id}`)
    revalidatePath(`/attraction/${id}/edit`)
    redirect('/attraction')
}
